...
public class CompanyController extends BaseController {
	...
	/*@Value("company.id")
	String companyid;*/
	@RequiresPermissions("web:company:company")
	@GetMapping("")
	String companyDO(Model model) {
		CompanyDO com=companyService.get(1l);
	...
	@RequiresPermissions("web:company:add")
	@Log("修改公司信息")
	@PostMapping("/save")
	@ResponseBody
	R save(CompanyDO com) {
		if(com.getIntrogg()!=null&&com.getIntrogg().length()>200) {
			com.setIntrogg(com.getIntrogg().substring(0, 200)+"...");
		}
		int count=0;
		if(com.getId()==null||(com.getId()+"").equals("")) {
			count=companyService.save(com);
		}else {
			count=companyService.update(com);
		}
		if (count > 0) {
			return R.ok();
		}
